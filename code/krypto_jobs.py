...
# YOUR CODE HERE
def display_balance():
    ether = get_balance(w3=w3, address=account.address)
...
if st.sidebar.button("Send Transaction"):
    ...
    # YOUR CODE HERE
    gas_price_gwei = 25  # Set the gas price to 50 Gwei
    transaction_hash = send_transaction(w3=w3, account=account, to=candidate_database[person][1], wage=wage, gas_price_gwei=gas_price_gwei)
    transaction_hash_str = transaction_hash.hex()

    # Markdown for the transaction hash
    st.sidebar.markdown("#### Validated Transaction Hash")

    # Write the returned transaction hash to the screen
    st.sidebar.write(transaction_hash_str)
